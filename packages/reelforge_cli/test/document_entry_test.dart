...
void main() {
  ...
  setUp(() {
    project = Directory.systemTemp.createTempSync('reelforge_doc');
    File('${project.path}/pubspec.yaml').writeAsStringSync(
      'name: demo\ndependencies:\n  reelforge_json: ^0.1.0\n',
    );
  ...
  group('resolving the host', () {
    test('without a document the project entry point is used untouched', () {
      ...
      expect(
        Directory('${project.path}/.dart_tool/reelforge').existsSync(),
        isFalse,
    ...
    test('a document generates an entry point under .dart_tool', () {
      ...
      expect(
        target.entryPoint,
        '.dart_tool/reelforge/document_render.dart',
      );
