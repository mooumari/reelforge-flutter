...
/// Adds ReelForge to an existing Flutter project.
///
...
Future<int> initCommand(CliArgs args) async {
  ...
  if (!original.contains('sdk: flutter')) {
    throw CliError(
      ...
      'ReelForge renders Flutter widgets, so it needs a Flutter project.',
    );
  }
  ...
  stdout.writeln(
    json
        ? '\nNext:\n'
            '  flutter pub get\n'
            '  reelforge preview video/reel.json '
            '--data video/reel_data.json$where\n'
            '\nand when it looks right:\n'
            '  reelforge render video/reel.json '
            '--data video/reel_data.json$where --out reel.mp4'
        : '\nNext:\n'
            '  flutter pub get\n'
            '  reelforge preview$where\n'
            '\nand when it looks right:\n'
            '  reelforge render$where --composition Intro --out intro.mp4',
  );
  return 0;
}
...
/// as `../reelforge/...`, not as somebody's home directory -- that is what
