...
///
/// The name has to exist in `fluttermotion_schema` first. That direction
...
void registerBuilder(String name, NodeBuilder build) {
  if (!knownNodeTypes.containsKey(name)) {
    throw StateError(
      ...
      'fluttermotion_schema first; a builder without one would render '
  ...
  Widget buildDirect(BuildContext context) {
    ...
    if (build == null) {
      throw StateError(
        ...
        'fluttermotion_json knows the type but cannot draw it.',
      );
    }
