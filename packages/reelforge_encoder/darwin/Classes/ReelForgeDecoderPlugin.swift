...
///
/// The counterpart to `ReelForgeEncoderPlugin`: the one platform-specific
...
public class ReelForgeDecoderPlugin: NSObject, FlutterPlugin {
  ...
  private let queue = DispatchQueue(label: "reelforge.decoder", qos: .userInitiated)
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let channel = FlutterMethodChannel(
      name: "reelforge/decoder", binaryMessenger: messenger)
    registrar.addMethodCallDelegate(ReelForgeDecoderPlugin(), channel: channel)
  }
